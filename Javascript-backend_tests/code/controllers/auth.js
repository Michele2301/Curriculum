...
export const register = async (req, res) => {
    try {
        ...
        if(!username.trim()||!email.trim()||!password.trim()) return res.status(400).json({error: "Missing required fields"})
...
export const login = async (req, res) => {
    ...
    if(!email.trim()||!password.trim()) return res.status(400).json({error: "Missing required fields"});
    if(!isValidEmail(email)) return res.status(400).json({error: "Invalid email format"});
